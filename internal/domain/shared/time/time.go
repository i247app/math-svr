...
func (mt MathTime) MarshalJSON() ([]byte, error) {
	...
	formatted := fmt.Sprintf("\"%s\"", mt.Format(Format20FSP))
...
func (mt MathTime) IsBefore(t time.Time) bool {
	return mt.Time.Before(t)
}

func (mt MathTime) IsAfter(t time.Time) bool {
	return mt.Time.After(t)
}

func (mt MathTime) IsEqual(t time.Time) bool {
	return mt.Time.Equal(t)
}
