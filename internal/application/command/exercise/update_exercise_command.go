...
func (h *UpdateClassroomExerciseCommandHandler) Handle(ctx context.Context, cmd UpdateClassroomExerciseCommand) (*domain.Exercise, error) {
	...
	err := h.uow.Do(ctx, func(ctx context.Context, repos transaction.Repositories) error {
		existing, err := repos.Exercise.FindByClassroomExerciseId(ctx, cmd.ClassroomExerciseID)
		if err != nil {
		...
		refreshed, err := repos.Exercise.FindByClassroomExerciseId(ctx, cmd.ClassroomExerciseID)
		if err != nil {
			return errs.NewError(ctx, status.FAIL, nil, err)
		}
		updated = refreshed
