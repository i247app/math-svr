...
func TestCacheTagIsStable(t *testing.T) {
	a := BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 10, "Học kỳ 1", "Cánh diều")
	b := BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 10, "Học kỳ 1", "Cánh diều")
	if a != b {
...
func TestCacheTagNormalisesInput(t *testing.T) {
	...
	want := BuildCacheTag(enum.ExamTypePractice, 2, level(5), 10, "Học kỳ 1", "Cánh diều")
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := BuildCacheTag(enum.ExamTypePractice, 2, level(5), 10, tc.semester, tc.program)
			if got != want {
...
func TestCacheTagKeepsItsArity(t *testing.T) {
	plain := BuildCacheTag(enum.ExamTypePractice, 1, level(1), 10, "HK1", "Canh dieu")
	hyphenated := BuildCacheTag(enum.ExamTypePractice, 1, level(1), 10, "HK1", "Chan troi - sang tao")
...
func TestCacheTagDistinguishesRequests(t *testing.T) {
	base := BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 10, "HK1", "CD")
	tests := []struct {
		name string
		tag  string
	}{
		{"different type", BuildCacheTag(enum.ExamTypePractice, 1, level(3), 10, "HK1", "CD")},
		{"different grade", BuildCacheTag(enum.ExamTypeAssessment, 2, level(3), 10, "HK1", "CD")},
		{"different level", BuildCacheTag(enum.ExamTypeAssessment, 1, level(4), 10, "HK1", "CD")},
		{"different length", BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 15, "HK1", "CD")},
		{"different semester", BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 10, "HK2", "CD")},
		{"different program", BuildCacheTag(enum.ExamTypeAssessment, 1, level(3), 10, "HK1", "KNTT")},
		{"no level at all", BuildCacheTag(enum.ExamTypeAssessment, 1, nil, 10, "HK1", "CD")},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if tc.tag == base {
...
func TestCacheTagMissingParts(t *testing.T) {
	got := BuildCacheTag(enum.ExamTypeExam, 0, nil, 10, "", "")
	want := "EXAM-G0-LNA-Q10-SNA-PNA"
	if got != want {
		t.Errorf("got %q, want %q", got, want)
	}
}
