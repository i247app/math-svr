...
#!/bin/sh
echo "Stopping mathsvr via systemd..."

STATUS=$(sudo systemctl is-active mathsvr.service 2>/dev/null)

if [ "$STATUS" = "active" ]; then
    echo "mathsvr is running. Stopping it..."
    sudo systemctl stop mathsvr.service
    echo "OK!"
elif [ "$STATUS" = "inactive" ]; then
    echo "mathsvr is already stopped."
else
    echo "mathsvr state: $STATUS (not running)"
fi